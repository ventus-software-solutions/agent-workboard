...
10. When the board reports no eligible work, inspect `upstreamSignal`: if its total is positive, heartbeat as `waiting`, wait `recheckAfterSeconds`, and re-poll. Stop only when the signal is zero, the operator pauses you, or a real blocker has been recorded.
...
After each PM task or grooming pass, look for the next eligible PM task. If none is eligible but `upstreamSignal.total` is positive, heartbeat as `waiting`, wait `recheckAfterSeconds`, and re-poll. Stop only when the signal is zero, an operator tells you to pause, or a decision requires operator input that you have recorded visibly.
```
...
After handoff, ask the board for the next backend task. If none is eligible but `upstreamSignal.total` is positive, heartbeat as `waiting`, wait `recheckAfterSeconds`, and re-poll. Stop when the signal is zero, the next candidate is sequenced for another specialty, or a recorded blocker requires operator input.
```
...
After implementation, run focused browser coverage, `npm test`, `npm run build`, and e2e when the change touches core flows. Commit, post branch/commit/test evidence, move the task to review, then ask for the next frontend task. If none is eligible but `upstreamSignal.total` is positive, heartbeat as `waiting`, wait `recheckAfterSeconds`, and re-poll. Stop when the signal is zero or a blocker has been recorded.
```
...
After each review outcome, scan `status=review` again. If no review is eligible but `upstreamSignal.total` is positive, heartbeat as `waiting`, wait `recheckAfterSeconds`, and re-poll. Stop when the signal is zero, a merge is blocked by permissions/conflicts you have recorded, or the operator tells you to pause.
```
...
After posting evidence, move the task to the requested next state, usually `review`, `testing`, `ready`, or `blocked` depending on the task. Then ask the board for the next tester task. If none is eligible but `upstreamSignal.total` is positive, heartbeat as `waiting`, wait `recheckAfterSeconds`, and re-poll. Stop when the signal is zero or a recorded blocker requires operator input.
```
...
Approve only with verification evidence. If changes are required, post specific findings, expected fixes, and risk level, then move the task back to `ready` or `blocked`. When no security review is eligible but `upstreamSignal.total` is positive, heartbeat as `waiting`, wait `recheckAfterSeconds`, and re-poll. Stop when the signal is zero or an operator decision is needed.
```
...
Verify with `git diff --check`, an ASCII scan when appropriate, and `npm run build` when docs are linked from app-facing docs. Commit, post evidence, move the task to review, and then ask for the next docs task. If none is eligible but `upstreamSignal.total` is positive, heartbeat as `waiting`, wait `recheckAfterSeconds`, and re-poll. Stop when the signal is zero.
```
...
After each handoff, ask for the next MCP task. If none is eligible but `upstreamSignal.total` is positive, heartbeat as `waiting`, wait `recheckAfterSeconds`, and re-poll. Stop when the signal is zero or a blocker has been recorded.
```
