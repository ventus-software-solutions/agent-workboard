...
function sharedWorkflow() {
  return [
    ...
    "After planning identifies likely files, record optional file or glob hints with MCP `update_task_touches` or the task `touches` field; overlap warnings are advisory and do not replace coordination.",
    ...
    "Fetch and rebase onto the current integration base before every delivery, then rerun relevant verification. If an overlapping task merges or Agent Talks reports an overlapping-file change, rebase again before continuing the handoff.",
