...
function decideIntegrationState({ ahead, behind, clean, localHead, originHead, localRef, remoteRef }) {
  ...
  if (!originHead) {
    return clean
      ? {
          ...
          baseRef: localRef,
          summary: `Origin ref ${remoteRef} is unavailable; use local ${localRef} as the task base until remote state is restored.`
        }
      : {
  ...
  if (ahead > 0 && behind > 0) {
    return {
      sourceOfTruth: "reconcile-first",
      baseRef: null,
      summary: `Local ${localRef} and ${remoteRef} have diverged (${ahead} ahead, ${behind} behind); reconcile before creating new task worktrees.`
    };
  }
  ...
  if (ahead > 0) {
    return clean
      ? {
          sourceOfTruth: "local-main",
          baseRef: localRef,
          summary: `Local ${localRef} is ${ahead} commit${ahead === 1 ? "" : "s"} ahead of ${remoteRef}; use local ${localRef} for task worktrees until push debt is cleared.`
        }
      : {
          sourceOfTruth: "reconcile-first",
          baseRef: null,
...
function recoveryActions({ pushDebt, sourceOfTruth, localRef, remoteRef }) {
  ...
  if (pushDebt) {
    return [
      ...
      "If push is unavailable, record push debt in the release/reviewer evidence and keep using local main as the task base.",
